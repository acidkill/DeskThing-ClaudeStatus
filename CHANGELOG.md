...
### Added
- `idle_reading` — Clawd reads an open book, eyes scan left-to-right and
  pages turn occasionally. 7 frames, 6-colour palette. Joins the idle pool.
- `work_blackboard` — Clawd writes physics formulas (F=ma, E=mc², a sine
  wave + Δ) on a chalkboard, with chalk-in-hand highlight and an erase
  smudge frame. 8 frames, 7-colour palette. Joins the busy pool.
- Both generated by the renamed `scripts/generate-sprites.mjs` (was
  `generate-idle-sprites.mjs`) so the build script now covers all custom
  Clawd animations.

### Changed
- **Renamed** `scripts/generate-idle-sprites.mjs` → `scripts/generate-sprites.mjs`
  to reflect the broader scope. Same overlay primitives and Clawd base pose
  shared across all custom sprites.
- **Demoted** `work_think` from category `Work` → `Archive`. The upstream
  sprite is kept in the repo for reference but no longer rotates in any
  mood pool. Busy pool is now `work_coding` (laptop) + `work_blackboard`
  (chalkboard).
- Idle pool grows from 5 → 6 (added reading).
- Busy pool unchanged in size but content shifted (work_think out, blackboard in).

### Notes on Opus vs Sonnet compatibility
The mood thresholds were calibrated for Sonnet 4.6 normal-use granularity.
Opus 4.7 burns budget ~5× faster per token, so the same user behaviour will
escalate mood more aggressively on Opus (busy / frantic reached in fewer
polls). This is intentional — the unified rate-limit headers are model-
agnostic, and an Opus user IS consuming the budget faster, so the mascot
should reflect that. If tighter tuning per model is desired, a future
`costSensitivity` setting could remap the tick-count and rate thresholds.
